...
Original file is located at
    https://colab.research.google.com/drive/18gJ-wFZE1jiBwbRINx5m_hYeqJYWqiU4

#Artificial Intelligence Final Report Assignment (Problem 3)
  ...
  for tokenlist in train_data:
    for token in tokenlist:
      if token not in vocab:
        vocab[token] = 0
      vocab[token] += 1 # データワードをカウントする
  vocablist = [('<unk>',0),('<pad>', 0), ('<cls>', 0), ('<eos>', 0)] # 以前に紹介したタプルのリスト
  vocabidx = {} # タプルのディクテーション
  for token, freq in vocab.items(): # そこから、トレーニング データ内の単語数で
    if freq >= min_freq: # 3 を>する単語数
      idx = len(vocablist) # それは4です。
      vocablist.append((token, freq)) #そこから、トレーニング データ内の単語数で
      vocabidx[token] = idx  # 辞書 (dict)
  vocabidx['<unk>'] = 0
  vocabidx['<pad>'] = 1
  vocabidx['<cls>'] = 2
  vocabidx['<eos>'] = 3
...
#入力インデックスのテンソルをトークン埋め込みの対応するテンソルに変換するヘルパーモジュール
...
def train():
  ...
  for epoch in range(50):
    ...
    if epoch %10 ==0:
      ...
      torch.save(model.state_dict(), "./transform2numE{}".format(epoch))
    torch.save(model.state_dict(), "./transform2numE50")
...
def test10():
  ...
  model.load_state_dict(torch.load("./transform2numE10"))
  model.eval()
  ref = []
  pred = []
...
def test20():
  model = Seq2SeqTransformer(NUM_ENCODER_LAYERS, NUM_DECODER_LAYERS, EMB_SIZE,
                                 NHEAD, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, FFN_HID_DIM).to(DEVICE)
  model.load_state_dict(torch.load("./transform2numE20"))
  model.eval()
  ref = []
  pred = []
  for enprep, en, vi in test_data:
...
def test30():
  model = Seq2SeqTransformer(NUM_ENCODER_LAYERS, NUM_DECODER_LAYERS, EMB_SIZE,
                                 NHEAD, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, FFN_HID_DIM).to(DEVICE)
  model.load_state_dict(torch.load("./transform2numE30"))
  model.eval()
  ref = []
  pred = []
  for enprep, en, vi in test_data:
...
def test40():
  model = Seq2SeqTransformer(NUM_ENCODER_LAYERS, NUM_DECODER_LAYERS, EMB_SIZE,
                                 NHEAD, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, FFN_HID_DIM).to(DEVICE)
  model.load_state_dict(torch.load("./transform2numE40"))
  model.eval()
  ref = []
  pred = []
  for enprep, en, vi in test_data:
...
def test50():
  model = Seq2SeqTransformer(NUM_ENCODER_LAYERS, NUM_DECODER_LAYERS, EMB_SIZE,
                                 NHEAD, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, FFN_HID_DIM).to(DEVICE)
  model.load_state_dict(torch.load("./transform2numE50"))
  model.eval()
  ref = []
  pred = []
  for enprep, en, vi in test_data:
